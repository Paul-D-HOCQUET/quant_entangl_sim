...
def IsingXX(phi, q1, q2, State):
    qml.AmplitudeEmbedding(features=State, wires=range(Nbq))
    qml.IsingXX(phi, wires=[q1, q2])
    return qml.state()
...
for q1 in range(Nbq):
    for q2 in range(Nbq):
        if q1 != q2 :
            etat_penny      = IsingXX (2*(3 * math.pi / 4), q1, q2, State)    
            etat_simul      = Porte_DeuxQubits_Param (Nbq, State, 'XX', q1, q2, eval("3 * math.pi / 4"))
